...
<script lang="ts">
    ...
    export default class ShopTihuo extends Vue {
//加载状态
        isLoading = false;
        //查询输入框
        searchStr: string = "";
        ...
        async query() {
            this.isLoading = true;
            this.tableData = await ShopHelper.queryOrderDetails(this.searchStr,2,this.page,this.row);
        ...
        async tiHuo() {
            ...
            let message = await ShopHelper.tiHuo(ids);
            if (message.flag) {
                this.$notify.success({title: "提示",message: "提货成功!"});
                this.query();
            } else {
